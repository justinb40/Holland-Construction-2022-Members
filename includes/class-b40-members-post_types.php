...
/**
 * The file is responsible for regsitering all custom post types.
 *
...
class B40_Members_Post_Types {
    ...
    private function register_single_post_type( $fields ) {
        ...
        $labels = array(
            ...
            'new_item'              => sprintf( __( 'New %s', 'b40-members' ), $fields['singular'] ),
            'add_new_item'          => sprintf( __( 'Add new %s', 'b40-members' ), $fields['singular'] ),
            'edit_item'             => sprintf( __( 'Edit %s', 'b40-members' ), $fields['singular'] ),
            'view_item'             => sprintf( __( 'View %s', 'b40-members' ), $fields['singular'] ),
            'view_items'            => sprintf( __( 'View %s', 'b40-members' ), $fields['plural'] ),
            'search_items'          => sprintf( __( 'Search %s', 'b40-members' ), $fields['plural'] ),
            'not_found'             => sprintf( __( 'No %s found', 'b40-members' ), strtolower( $fields['plural'] ) ),
            'not_found_in_trash'    => sprintf( __( 'No %s found in trash', 'b40-members' ), strtolower( $fields['plural'] ) ),
            'all_items'             => sprintf( __( '%s', 'b40-members' ), $fields['plural'] ),
            'archives'              => sprintf( __( '%s Archives', 'b40-members' ), $fields['singular'] ),
            'attributes'            => sprintf( __( '%s Attributes', 'b40-members' ), $fields['singular'] ),
            'insert_into_item'      => sprintf( __( 'Insert into %s', 'b40-members' ), strtolower( $fields['singular'] ) ),
            'uploaded_to_this_item' => sprintf( __( 'Uploaded to this %s', 'b40-members' ), strtolower( $fields['singular'] ) ),

            /* Labels for hierarchical post types only. */
            'parent_item'           => sprintf( __( 'Parent %s', 'b40-members' ), $fields['singular'] ),
            'parent_item_colon'     => sprintf( __( 'Parent %s:', 'b40-members' ), $fields['singular'] ),
        ...
        $args = array(
            'labels'             => ( isset( $fields['labels'] ) ) ? $fields['labels'] : $labels,
        ...
        /**
         * Register Statuses if any
         */
        if ( isset( $fields['statuses'] ) && is_array( $fields['statuses'] ) ) { 
            
            foreach ( $fields['statuses'] as $status ) {

                $this->register_single_post_type_status( $status );
    ...
    private function register_single_post_type_taxnonomy( $tax_fields ) {
        ...
        $labels = array(
            ...
            'all_items'                  => sprintf( __( 'All %s' , 'b40-members' ), $tax_fields['plural'] ),
            'edit_item'                  => sprintf( __( 'Edit %s' , 'b40-members' ), $tax_fields['single'] ),
            'view_item'                  => sprintf( __( 'View %s' , 'b40-members' ), $tax_fields['single'] ),
            'update_item'                => sprintf( __( 'Update %s' , 'b40-members' ), $tax_fields['single'] ),
            'add_new_item'               => sprintf( __( 'Add New %s' , 'b40-members' ), $tax_fields['single'] ),
            'new_item_name'              => sprintf( __( 'New %s Name' , 'b40-members' ), $tax_fields['single'] ),
            'parent_item'                => sprintf( __( 'Parent %s' , 'b40-members' ), $tax_fields['single'] ),
            'parent_item_colon'          => sprintf( __( 'Parent %s:' , 'b40-members' ), $tax_fields['single'] ),
            'search_items'               => sprintf( __( 'Search %s' , 'b40-members' ), $tax_fields['plural'] ),
            'popular_items'              => sprintf( __( 'Popular %s' , 'b40-members' ), $tax_fields['plural'] ),
            'separate_items_with_commas' => sprintf( __( 'Separate %s with commas' , 'b40-members' ), $tax_fields['plural'] ),
            'add_or_remove_items'        => sprintf( __( 'Add or remove %s' , 'b40-members' ), $tax_fields['plural'] ),
            'choose_from_most_used'      => sprintf( __( 'Choose from the most used %s' , 'b40-members' ), $tax_fields['plural'] ),
            'not_found'                  => sprintf( __( 'No %s found' , 'b40-members' ), $tax_fields['plural'] ),
        );
    ...
    private function register_single_post_type_status( $status_fields ) {

        $args = array(
            'label'                     => $status_fields['label'],
            'public'                    => $status_fields['public'],
            'label_count'               => $status_fields['label_count'],
            'post_types'                => $status_fields['post_types'],
            'show_in_admin_all_list'    => $status_fields['show_in_admin_all_list'],
            'show_in_admin_status_list' => $status_fields['show_in_admin_status_list'],
            'show_in_metabox_dropdown'  => $status_fields['show_in_metabox_dropdown'],
            'show_in_inline_dropdown'   => $status_fields['show_in_inline_dropdown'],
            'dashicon'                  => $status_fields['dashicon'],
        );

        register_post_status( $status_fields['status'],  $args );
    ...
    public function create_custom_post_type() {
        ...
        /**
         * This is not all the fields, only what I find important.
         *
         * @link https://codex.wordpress.org/Function_Reference/register_post_type
         *
         * For more info on fields:
         * @link https://github.com/JoeSz/WordPress-Plugin-Boilerplate-Tutorial/blob/9fb56794bc1f8aebfe04e99b15881db0c4bc61bd/b40-members/includes/class-b40-members-post_types.php#L230
         */
